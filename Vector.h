...
public:
	...
	Vector();
	Vector(T init); //define by length, ;
...
template<typename T, size_t lengths>
Vector<T, lengths>::Vector():length_(lengths),Matrix<T,lengths,1>(0.){} //default constructor

template<typename T, size_t lengths>
Vector<T, lengths>::Vector(T init):
	Matrix<T, lengths, 1>(init),
	length_(lengths){} //assign constructor

template<typename T, size_t lengths>
Vector<T, lengths>::Vector(const Vector<T, lengths>& orig):
	Matrix<T, lengths, 1>(0.),length_(orig.length_)
...
Vector<T, lengths>::Vector(int length, std::function<T*(int)> f) :
	length_(length), Matrix<T, lengths, 1>(0.)
{
	T* p = f(length_);
//	this->p_ = new T[length_];
//	this->sizeX_ = length_;
...
template<typename T, size_t lengths>
T& Vector<T, lengths>::operator()(int index) {
